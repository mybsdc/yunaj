...
function tinyImgUpload(ele, options) {
    ...
    if(eleList.length == 0){
        ...
        eleList[0].innerHTML ='<div id="img-container" >'+
                '<div class="img-up-add select-btn"> <div class="btn btn-primary" style="border-radius: 22px;">选择照片</div></div>'+
                '<input type="file" name="image[]" id="img-file-input" multiple>'+
                '</div>';
                ...
                reader.onload = (function (theFile) {
                    return function (e) {
                        ...
                        oDiv.innerHTML = '<img class="thumb-icon" src="'+e.target.result+'" />'+
                                        '<a href="javscript:;" class="img-remove ion-android-cancel"></a>'
    ...
    function uploadImg() {
        ...
        for(var i=0, f; f=ele.files[i]; i++){
            formData.append('image[]', f); // 异步提交文件name
        }
