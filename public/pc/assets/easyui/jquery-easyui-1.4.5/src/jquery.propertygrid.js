...
(function($){
	...
	function stopEditing(target){
		...
		opts.finder.getTr(target, null, 'editing').each(function(){
			var index = parseInt($(this).attr('datagrid-row-common'));
	...
	var groupview = $.extend({}, $.fn.datagrid.defaults.view, {
		...
		renderGroup: function(target, groupIndex, group, frozen){
			...
			var table = [];
			table.push('<div class="datagrid-group" group-common=' + groupIndex + '>');
			...
			for(var j=0; j<group.rows.length; j++) {
				...
				table.push('<tr id="' + rowId + '" datagrid-row-common="' + index + '" ' + cls + ' ' + style + '>');
		...
		bindEvents: function(target){
			...
			body.unbind('click').bind('click', function(e){
				...
				if (expander.length){
					var gindex = expander.closest('div.datagrid-group').attr('group-common');
	...
	$.extend($.fn.datagrid.methods, {
	    ...
	    expandGroup:function(jq, groupIndex){
	        return jq.each(function(){
	            ...
	            var group = view.find(groupIndex!=undefined ? 'div.datagrid-group[group-common="'+groupIndex+'"]' : 'div.datagrid-group');
	...
	$.extend(groupview, {
		refreshGroupTitle: function(target, groupIndex){
			...
			var span = dc.body2.children('div.datagrid-group[group-common=' + groupIndex + ']').find('span.datagrid-group-title');
		...
		updateRow: function(target, index, row){
			...
			var groupIndex = parseInt(tb.prev().attr('group-common'));
			this.refreshGroupTitle(target, groupIndex);
		},
		...
		deleteRow: function(target, index){
			...
			if (group.rows.length > 1){
				...
				body.children('div.datagrid-group[group-common='+groupIndex+']').remove();
				for(var i=groupIndex+1; i<this.groups.length; i++){
					body.children('div.datagrid-group[group-common='+i+']').attr('group-common', i-1);
				}
