<?php if (!defined('THINK_PATH')) exit(); /*a:3:{s:33:"app/mobile\view\my_log\index.html";i:1498102325;s:32:"app/mobile\view\common\base.html";i:1497450644;s:34:"app/mobile\view\common\bottom.html";i:1497489320;}*/ ?>
<!DOCTYPE html>
<html>
    ...
    <div class="top">
        ...
        <script type="text/javascript">
            $('#toSearch').bind('input propertychange', function() { // 监听搜索框
                ...
                var html = '';
                var f = 1; // 隔行换色
                $.get("<?php echo url('my_log/index'); ?>", {num: num}, function(data){
                    ...
                    for (var i = 0; i < dataObj.length; i++) {
                        ...
                        html += '<a href="<?php echo url('mobile/user_info/index'); ?>?cstcode=' + dataObj[i].cstcode + '&check_id=' + dataObj[i].check_id + '&status=' + dataObj[i].status + '&checkuserid=<?php echo session('checkuserid'); ?>&from=myLog" class="log-limit">';
...
<div class="room">
    <div class="room-select">
        ...
        $('#time-btn').click(function(){
            ...
            $.get("<?php echo url('my_log/index'); ?>", {startTime: getStartTime, endTime: getEndTime, fa: isAreaID, fx: isXqID}, function(data){
                /*let dataObj = eval(data); // json转对象
                var html = '';
                var f = 1; // 隔行换色
                for (var i = 0; i < dataObj.length; i++) {
    ...
    laypage({
        ...
        jump: function(obj, first){
            ...
            var isAreaID = $('#getAreaID').attr('class'); // 区域id，如果有的话
            var isXqID = $('#getXqID').attr('class'); // 小区id，如果有的话
            $.get("<?php echo url('my_log/index'); ?>", {diytime: curr, fstime: getStartTime, fetime: getEndTime, ffa: isAreaID, ffx: isXqID}, function(data){ // 第二次与首次ajax区分下，否则分支会跑错，因为翻页是不会有任何选择时间事件的
                let dataObj = eval(data); // json转对象
                var html = '';
                var f = 1; // 隔行换色
                for (var i = 0; i < dataObj.length; i++) {
...
layui.use(['laypage', 'layer'], function(){ // 模块化使用layer
    ...
    laypage({
        ...
        jump: function(obj, first){
            ...
            // 得到了当前页，用于向服务端请求对应数据
            var curr = obj.curr;
            $.get("<?php echo url('my_log/index'); ?>", {page: curr}, function(data){
                let dataObj = eval(data); // json转对象
                var html = '';
                var f = 1; // 隔行换色
                for (var i = 0; i < dataObj.length; i++) {
...
$('#area').click(function() {
        ...
        $.ajax({
            ...
            success: function(data) {
                ...
                for (var i = 0; i < data['log'].length; i++) {
                    ...
                    html2 += '<a href="<?php echo url('mobile/user_info/index'); ?>?cstcode=' + data['log'][i].cstcode + '&check_id=' + data['log'][i].check_id + '&status=' + data['log'][i].status + '&checkuserid=<?php echo session('checkuserid'); ?>&from=myLog" class="log-limit">';
    ...
    laypage({
        ...
        jump: function(obj, first){
            ...
            // 得到了当前页，用于向服务端请求对应数据
            var curr = obj.curr;
            $.get("<?php echo url('my_log/index'); ?>", {aa: curr, aaid: id}, function(data){
                let dataObj = eval(data); // json转对象
                var html = '';
                var f = 1; // 隔行换色
                for (var i = 0; i < dataObj.length; i++) {
...
$('#xq').click(function() {
        ...
        $.ajax({
            ...
            success: function(data) {
                ...
                for (var i = 0; i < data.length; i++) {
                    ...
                    html += '<a href="<?php echo url('mobile/user_info/index'); ?>?cstcode=' + data[i].cstcode + '&check_id=' + data[i].check_id + '&status=' + data[i].status + '&checkuserid=<?php echo session('checkuserid'); ?>&from=myLog" class="log-limit">';
                        html += '<span class="log-left">';
    ...
    laypage({
        ...
        jump: function(obj, first){
            ...
            // 得到了当前页，用于向服务端请求对应数据
            var curr = obj.curr;
            $.get("<?php echo url('my_log/index'); ?>", {xx: curr, xxid: id}, function(data){
                let dataObj = eval(data); // json转对象
                var html = '';
                var f = 1; // 隔行换色
                for (var i = 0; i < dataObj.length; i++) {
...
time.click(function(){
    ...
    timeAll.find('li').click(function(){
        ...
        var isAreaID = $('#getAreaID').attr('class'); // 区域id，如果有的话
        var isXqID = $('#getXqID').attr('class'); // 小区id，如果有的话
        $.get("<?php echo url('my_log/index'); ?>", {theTime: theTime, myArea: isAreaID, myXq: isXqID}, function(data){
            // alert(data)
            /*let dataObj = eval(data); // json转对象
            var html = '';
            var f = 1; // 隔行换色
...
var wCountPages = data; // 后台返回的总页数
    ...
    laypage({
        ...
        jump: function(obj, first){
            ...
            $.get("<?php echo url('my_log/index'); ?>", {ww: curr, wwid : theTime, getAID: isAreaID, getXID: isXqID}, function(data){
                let dataObj = eval(data); // json转对象
                var html = '';
                var f = 1; // 隔行换色
                for (var i = 0; i < dataObj.length; i++) {
