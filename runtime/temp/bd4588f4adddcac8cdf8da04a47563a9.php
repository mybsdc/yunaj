<?php if (!defined('THINK_PATH')) exit(); /*a:2:{s:37:"app/home\view\audit_check\addlog.html";i:1498704997;s:30:"app/home\view\common\base.html";i:1498449249;}*/ ?>
﻿<!DOCTYPE html>
...
<form action="#" method="post" class="room-info" enctype="multipart/form-data" id="goToPost">
    ...
    <!-- <div class="col-sm-12" style="border: 1px solid #307ecc; height: 200px;"></div> -->
    
    <div class="col-sm-12">
        <ul id="problem">
        <?php if(is_array($problem) || $problem instanceof \think\Collection || $problem instanceof \think\Paginator): $i = 0; $__LIST__ = $problem;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 1 );++$i;?>
        ...
        <input type="hidden" name="cstname" value="<?php echo $roomInfo['cstname']; ?>" />
        <input type="hidden" name="telphone" value="<?php echo $roomInfo['telphone']; ?>" />
        <input type="hidden" name="type" value="<?php echo $roomInfo['type']; ?>" />
        <input type="hidden" name="brand" value="<?php echo $roomInfo['brand']; ?>" />
        <input type="hidden" name="direction" value="<?php echo $roomInfo['direction']; ?>" />
        <input type="hidden" name="basenumber" value="<?php echo $roomInfo['basenumber']; ?>" />
    ...
    <div class="col-sm-12" style="text-align: center;">
        <button class="btn btn-primary" id="doPost">提交</button>
    </div>
    ...
    <script type="text/javascript">
        $('#doPost').click(function(){ // 点击提交
            $('#doPost').html('正在提交');
            $.post("<?php echo url('home/audit_check/doAddLog'); ?>", $('#goToPost').serialize(), function(data){ // 异步post，方便加tips
                if (data) {
                    swal('恭喜，提交成功', '请等待复审结果', 'success');
                    $('#doPost').html('提交');
                    setTimeout(function() {
                        window.location.href = "<?php echo url('home/audit_check/index'); ?>";
                    }, 1000)
                } else {
