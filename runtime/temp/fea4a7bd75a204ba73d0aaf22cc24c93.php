<?php if (!defined('THINK_PATH')) exit(); /*a:2:{s:33:"app/home\view\do_check\index.html";i:1498787754;s:30:"app/home\view\common\base.html";i:1498787932;}*/ ?>
﻿<!DOCTYPE html>
...
<li class="">
    ...
    <?php if(is_array($res) || $res instanceof \think\Collection || $res instanceof \think\Paginator): if( count($res)==0 ) : echo "" ;else: foreach($res as $key=>$vo): switch($vo['id']): case "1": ?>
                <li f_id="<?php echo $vo['id']; ?>" class="active">
                    <a href="<?php echo url('home/do_check/index'); ?>">
                        <i class="icon-edit"></i>
...
<div class="row" style="background-color: #f0f3f8; padding: 16px 0; height: 55px;">
    ...
    <script type="text/javascript">
    	...
    	$('#city').change(function(){
    		...
    		$.post("<?php echo url('home/do_check/index'); ?>", {city_id: city_id}, function(data){
			    ...
			    layui.laypage({
			        ...
			        jump: function(obj, first){
				        ...
				        $.post("<?php echo url('home/do_check/toPage'); ?>", postData, function(data){
			                ...
			                $(dataObj).each(function(i){
			                	html += '<tr class="' + this.id + '">';
					                ...
					                html += '<td class="hidden-480 center"><a href="<?php echo url('home/do_check/addLog'); ?>?cstcode=' + this.cstcode + '"><i class="ion-ios-plus-outline" style="color: #2f7bba; font-size: 18px; cursor: pointer"></i></a></td>';
					            html += '</tr>';
			                });
    	...
    	$('#area').change(function(){
    		...
    		var html = '<option value="">请选择</option>';
    		$.post("<?php echo url('home/do_check/index'); ?>", {area_id: area_id}, function(data){
    			// let dataObj = eval(data); // json转对象
    	...
    	$('#xq').change(function(){
    		var xq_id = $(this).val();
    		var html = '';
    		$.post("<?php echo url('home/do_check/index'); ?>", {xq_id: xq_id}, function(data){
    			// let dataObj = eval(data); // json转对象
    			var dataObj = JSON.parse(data);
				$('#pageNum').html(dataObj.pageNum); // 共几页
				$('#isCount').html(dataObj.count); // 共几条
    	...
    	$('#task').change(function(){
    		...
    		var html = '';
    		$.post("<?php echo url('home/do_check/index'); ?>", {task_id: task_id}, function(data){
    			var dataObj = JSON.parse(data);
				$('#pageNum').html(dataObj.pageNum); // 共几页
				$('#isCount').html(dataObj.count); // 共几条
			    layui.laypage({
...
<script type="text/javascript">
    $('#toSearch').bind('input propertychange', function() { // 监听搜索框
        ...
        var html = '';
        $.get("<?php echo url('home/do_check/index'); ?>", {num: num}, function(data){
        	var dataObj = JSON.parse(data);
			$('#pageNum').html(dataObj.pageNum); // 共几页
			$('#isCount').html(dataObj.count); // 共几条
		    layui.laypage({
...
<div class="table-responsive">
        ...
        <?php if(is_array($listData) || $listData instanceof \think\Collection || $listData instanceof \think\Paginator): $i = 0; $__LIST__ = $listData;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?>
            <tr class="<?php echo $vo['id']; ?>">
                ...
                <td class="hidden-480 center">
	                <a href="<?php echo url('home/do_check/addLog'); ?>?cstcode=<?php echo $vo['cstcode']; ?>"><i class="ion-ios-plus-outline" style="color: #2f7bba; font-size: 18px; cursor: pointer"></i></a>
                </td>
            </tr>
        <?php endforeach; endif; else: echo "" ;endif; ?>
    ...
    <script type="text/javascript">
		$(document).delegate('#toExcel', 'click', function(){ // 导出excel
			...
			/*$.get("<?php echo url('home/do_check/toExcel'); ?>", {toCityId: toCityId, toAreaId: toAreaId, toXqId: toXqId}, function (data){
					console.log(data)
			}, 'json');*/
			window.location.href="<?php echo url('home/do_check/toExcel'); ?>?toCityId=" + toCityId + "&toAreaId=" + toAreaId + "&toXqId=" + toXqId + "&num=" + num + "&toTask=" + toTask;
		});
	</script>
