...
class CheckInfo extends Model
{
    ...
    {
        $data = db()->query("SELECT b.name, a.checktime, a.status, a.id, a.checkuserid FROM yunaj_checkinfo a JOIN yunaj_user b ON b.id = a.checkuserid WHERE a.cstcode = $cstcode");
        foreach ($data as $v){
        ...
        if (!empty($wmy)){
            ...
            if ($wmy === 'w'){ // 本周
                ...
                */
                ...
                $myReportData = db()->query($sql);
                $myReportData[0]['havePro'] = db('checkinfo')->alias('a')->join('yunaj_checkproblem b','a.id = b.check_id')->group('b.answer, b.check_id')->where(['a.checkuserid' => $checkuserid, 'b.answer' => 0])->count('a.id'); // 有安全隐患总数，仅统计总数时field非必需 */
                $week = "AND WEEK('c.checktime') = WEEK(NOW())";
                ...
                $month = "AND MONTH('c.checktime') = MONTH(NOW())";
                ...
                $year = "AND YEAR('c.checktime') = YEAR(NOW())";
